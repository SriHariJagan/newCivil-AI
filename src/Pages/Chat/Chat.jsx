...
const Chat = () => {
  ...
  const sendMessage = async (customText = null) => {
    ...
    try {
      ...
      setTypingMessage({ ...botMessage, conversationId: targetConversationId });
    } catch (error) {
          ...
          {typingMessage && !loading && (
            <div className={`${styles.messageReceived} ${styles.messageText}`}>
              <TypingText
                ...
                speed={20}
                onComplete={() => {
                  if (typingMessage.conversationId === activeConversation?.id) {
                    setMessages((prev) => [...prev, typingMessage]);
                  } else {
                    const stored = localStorage.getItem(`chat-${typingMessage.conversationId}`);
                    const parsed = stored ? JSON.parse(stored) : [];
                    localStorage.setItem(
                      `chat-${typingMessage.conversationId}`,
                      JSON.stringify([...parsed, typingMessage])
                    );
                  }
